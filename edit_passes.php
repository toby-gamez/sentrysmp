...
$db = new PDO("sqlite:battlepasses.sqlite");
$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["delete"])) {
        $stmt = $db->prepare("DELETE FROM Battlepasses WHERE id = ?");
        ...
        $sales =
            isset($_POST["sales"]) && $_POST["sales"] !== ""
                ...
                : null;
        $stmt = $db->prepare(
            "UPDATE Battlepasses SET name = ?, value = ?, prikaz = ?, price = ?, sales = ? WHERE id = ?",
        );
        $stmt->execute([
            $_POST["name"],
            $_POST["value"],
        ...
        $sales =
            isset($_POST["new_sales"]) && $_POST["new_sales"] !== ""
                ...
                : null;
        $stmt = $db->prepare(
            "INSERT INTO Battlepasses (name, value, image, prikaz, price, sales) VALUES (?, ?, ?, ?, ?, ?)",
        );
        $stmt->execute([
            $_POST["new_name"],
            $_POST["new_value"],
...
$keys = $db->query("SELECT * FROM Battlepasses")->fetchAll(PDO::FETCH_ASSOC);
...
<html>
    <head>
        ...
        <title>Edit Battlepasses - Sentry SMP</title>
        <meta charset="utf-8" />
    ...
    <body>
        ...
        <div class="container">
            <div class="main-wrapper">
                <h1 class="main">Edit Battlepasses</h1>
            </div>
            <ul class="edit-keys">
                <?php foreach ($keys as $k): ?>
                    <li class="edit-key">
                        ...
                        <form method="post" class="edit-form">
                            ...
                            <button type="submit" name="update" onclick="showNotification('Updating battlepass...', 'info', 1500)">Edit</button>
    ...
    $k["name"],
            ...
            <div class="main-wrapper">
            <h1 class="main">Add Battlepass</h1>
            </div>
                    ...
                    window.addEventListener('DOMContentLoaded', function() {
                        ...
                        <?php if ($_SERVER["REQUEST_METHOD"] === "POST"): ?>
                            <?php if (isset($_POST["delete"])): ?>
                                showNotification("Battlepass successfully deleted", "success");
                            <?php elseif (isset($_POST["update"])): ?>
                                showNotification("Battlepass successfully updated", "success");
                            <?php elseif (isset($_POST["add"])): ?>
                                showNotification("New battlepass successfully added", "success");
                            <?php endif; ?>
                        <?php endif; ?>
                        ...
                        document.querySelectorAll('.delete-btn').forEach(button => {
                            button.addEventListener('click', function(e) {
                                ...
                                showConfirmDialog(
                                    `Are you sure you want to delete the battlepass "${keyName}"?<br>This action cannot be undone.`,
                                    function() {
                                        ...
                                        showNotification("Deleting battlepass...", "info", 2000);
                                        form.submit();
                                    },
                                    function() {
                        ...
                        document.querySelectorAll('.edit-form').forEach(form => {
                            form.addEventListener('submit', function(e) {
                                if (this.querySelector('[name="update"]')) {
                                    showNotification("Updating battlepass...", "info", 2000);
                                }
                            });
                        });
                        ...
                        if (addForm) {
                            addForm.addEventListener('submit', function() {
                                showNotification("Adding new battlepass...", "info", 2000);
                            });
                        }
                    });
                </script>
